    ...
    def create_creature(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power == "Goblin":
            return (CreatureCard("Goblin Warrior", 2,
                                 Rarity.LEGENDARY.value, 5, 8))
        else:
            return CreatureCard("Fire Dragon", 5,
                                Rarity.LEGENDARY.value, 12, 12)

    def create_spell(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power:
            return SpellCard(name_or_power, 4,
                             Rarity.RARE.value, "hits the enemy")
        return SpellCard("Lightning bolt", 3,
                         Rarity.RARE.value, "shocks the enemy")

    def create_artifact(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power:
            return ArtifactCard(name_or_power, 4, Rarity.RARE.value, 999,
                                "is now on the field")

        return ArtifactCard("Mana Crystal", 4, Rarity.RARE.value, 999,
